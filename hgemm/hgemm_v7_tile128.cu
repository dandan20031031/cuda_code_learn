...
// Block tile: shared memory 级别（大 tile 版）
#define BM 128
#define BN 128
#define BK 32

// padding: 打破 shared memory bank 对齐（关键！）
#define BK_PAD (BK + 8)   // 40 halfs = 80 字节 = 20 bank，消除 A 的 8 路冲突
#define BN_PAD (BN + 8)   // 136 halfs = 272 字节 = 68 bank，消除 B 的 16 路冲突

#define WARPS_M 2
#define WARPS_N 4
#define WARPS   (WARPS_M * WARPS_N)      // 8 warps

#define COARSE_M (BM / WMMA_M / WARPS_M) // 4
                                              ...
                                              int N, int n_off, int kb)
{
    ...
    constexpr int CHUNKS_PER_ROW = BN / 8;        // 每行 128 个 half = 16 个 16 字节块
...
__global__ void hgemm_wmma_tile128(
    const half *A,
    const half *B,
    float       *C,
    int M, int N, int K)
{
    ...
    for (int it = 0; it < n_iters; it++) {
        ...
        // ---- 计算：先一次性加载本 warp 所需的 A/B fragment，再做 MMA ----
        #pragma unroll
...
int main() {
    ...
    printf("\n=== WMMA cp.async + Padding HGEMM (tile 128x128, 8 warps) ===\n");
    printf("  M=N=K=%d\n", M);
    ...
    hgemm_wmma_tile128<<<grid, block>>>(dA, dB, dC, M, N, K);
